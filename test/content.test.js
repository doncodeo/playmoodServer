...
describe('Content API', function() {
  ...
  describe('PUT /api/content/approve/:id', () => {
    ...
    describe('PUT /api/content/:id/like', () => {
        ...
        it('should like a content item and broadcast a websocket event', (done) => {
            let wsClient;
            let messages = [];
                ...
                request(app)
                    ...
                    .end((err, res) => {
                        ...
                        setTimeout(() => {
                            ...
                            if (wsMessage.event !== 'content_liked') {
                                return done(new Error('Incorrect WebSocket event'));
                            }
                            if (wsMessage.payload.contentId !== content._id.toString()) {
                                return done(new Error('Incorrect content ID in WebSocket payload'));
                            }
    ...
    describe('PUT /api/content/:id/unlike', () => {
        ...
        it('should unlike a content item and broadcast a websocket event', (done) => {
            let wsClient;
            let messages = [];
                ...
                request(app)
                    ...
                    .end((err, res) => {
                        ...
                        setTimeout(() => {
                            ...
                            const wsMessage = messages[0];
                            if (wsMessage.event !== 'content_unliked') {
                                return done(new Error('Incorrect WebSocket event'));
                            }
                            if (wsMessage.payload.contentId !== content._id.toString()) {
                                return done(new Error('Incorrect content ID in WebSocket payload'));
    ...
    describe('POST /api/content/:id/comment', () => {
        let content;
        let wsClient;
        let messages = [];
        ...
        it('should add a comment and broadcast a websocket event', (done) => {
            request(app)
                .post(`/api/content/${content._id}/comment`)
                .set('Authorization', `Bearer ${token}`)
                .send({ text: 'This is a test comment' })
                .expect(201)
                .end((err, res) => {
                    if (err) return done(err);
                    if (res.body.message !== 'Comment added successfully!') {
                        return done(new Error('Incorrect comment message'));
                    }
                    if (res.body.comment.text !== 'This is a test comment') {
                        return done(new Error('Comment text does not match'));
                    }
                    ...
                    setTimeout(() => {
                        ...
                        const wsMessage = messages[0];
                        if (wsMessage.event !== 'comment_added') {
                            return done(new Error('Incorrect WebSocket event'));
                        }
                        if (wsMessage.payload.contentId !== content._id.toString()) {
                            return done(new Error('Incorrect content ID in WebSocket payload'));
                        }
                        if (wsMessage.payload.comment.text !== 'This is a test comment') {
                            return done(new Error('Incorrect comment text in WebSocket payload'));
                        }
                        done();
                    }, 500);
                });
